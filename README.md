...
- 💰 **Биллинг** и управление балансом пользователей
...
После установки автоматически показывается меню управления ботом. Доступ в любой момент:
...
1. **📊 Статус бота** - проверка работы службы и время работы
2. **🌍 Список серверов** - просмотр всех подключенных серверов 3x-UI
3. **🔄 Перезапустить бота** - применение изменений конфигурации
4. **📋 Показать логи** - последние 50 строк логов
5. **⚙️ Редактировать конфигурацию** - изменение `.env` через nano
6. **🗑️ Полное удаление бота** - удаление с двойным подтверждением и прощанием
0. **👋 Выход** - красивое прощание с QR-кодом доната

### 🔄 Автоматическое обновление

При повторном запуске скрипта:
- Автоматическая проверка новой версии на GitHub
- Предложение обновить установщик
- Сохранение всех настроек при обновлении
- Выбор действия: открыть меню, переустановить или обновить только код

### 💚 Система благодарности

QR-коды для поддержки разработчика показываются:
- ✅ После успешной установки
- ✅ При выходе из меню (пункт 0)
- ✅ При выходе после повторного запуска (пункт 4)
- ✅ При удалении бота (пункт 6) - **перед** удалением файлов
...
#### 📡 Мониторинг:
- **Статус службы** - systemctl status с деталями
- **Просмотр логов** - последние 50 строк или в реальном времени
- **Время работы** - uptime бота

#### ⚙️ Настройка:
- **Редактирование .env** - через встроенный nano
- **Список серверов** - все подключенные панели 3x-UI
- **Перезапуск** - применение изменений

#### 🗑️ Удаление:
- Двойное подтверждение (YES + УДАЛИТЬ)
- Благодарность и QR-код **перед** удалением
- Полная очистка всех файлов и служб
...
```bash
vpn-bot
# Выбрать: 5 (Редактировать конфигурацию)
# Добавить новый блок сервера в .env
# Сохранить (Ctrl+O, Enter, Ctrl+X)
# Выбрать: 3 (Перезапустить)
```
...
/root/vpn-bot/
├── vpn_bot.py              # Код бота
├── menu.sh                 # Скрипт меню управления
├── .env                    # Конфигурация (защищен chmod 600)
└── vpn-bot-env/            # Python виртуальное окружение
    ├── bin/
    │   └── python          # Python интерпретатор
    └── lib/                # Установленные пакеты

/usr/local/bin/
└── vpn-bot                 # Команда быстрого доступа

/etc/systemd/system/
└── vpn-bot.service         # Системная служба
```

## 🔄 Повторный запуск скрипта

При запуске на уже установленном боте показывается меню:

```
╔═══════════════════════════════════════════════════════╗
║                                                       ║
║     ✅ VPN TELEGRAM BOT УЖЕ УСТАНОВЛЕН!              ║
║                                                       ║
╚═══════════════════════════════════════════════════════╝
...
1) Открыть меню управления
2) Переустановить бота (удалит текущую конфигурацию)
3) Обновить только код бота (сохранит настройки)
4) Выход
```

### Варианты действий:

- **1 - Открыть меню** - доступ к функциям управления
- **2 - Переустановка** - полная переустановка с нуля (с подтверждением YES)
- **3 - Обновление кода** - обновление только `vpn_bot.py` с сохранением `.env`
- **4 - Выход** - прощание с QR-кодом доната
...
# Server 1 - Германия
XUI_HOST_1=https://84.21.173.216:17228
XUI_PATH_1=/kDYLDAOQis3aMfA
XUI_USERNAME_1=RjI4h1XLt9
XUI_PASSWORD_1=your_password
XUI_TOKEN_1=
SERVER_IP_1=84.21.173.216
...
**🔗 T-Bank (Тинькофф):** https://www.tbank.ru/cf/A1Cj74Nvan6
...
- 💚 **Донат**: [T-Bank](https://www.tbank.ru/cf/A1Cj74Nvan6)

## 📜 Changelog

### v3.4 (2025-12-03)
- ✅ Добавлено интерактивное меню управления
- ✅ Автоматическая проверка обновлений
- ✅ QR-коды для поддержки разработчика
- ✅ Исправлена ошибка curl: (23)
- ✅ Улучшен процесс удаления с прощанием
- ✅ Добавлена команда vpn-bot для быстрого доступа
- ✅ Установка qrencode для отображения QR-кодов

### v3.0-3.3
- Базовая автоматическая установка
- Поддержка мультисервер
- Systemd служба
...
**💚 Поддержите разработчика:** https://www.tbank.ru/cf/A1Cj74Nvan6
